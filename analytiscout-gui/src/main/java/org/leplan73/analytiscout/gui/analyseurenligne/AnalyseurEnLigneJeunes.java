...
public class AnalyseurEnLigneJeunes extends AnalyseurEnLigne {
	...
	public AnalyseurEnLigneJeunes() {
		super("Analyseur en ligne (Jeunes)", logger_, new File(Preferences.lit(Consts.REPERTOIRE_SORTIE, "données", false),"analyse_jeunes.xlsx"), new File(Preferences.lit(Consts.REPERTOIRE_SORTIE, "données", false)), Consts.NOM_FICHIER_ANALYSE_JEUNES, new File("conf/batch_jeunes.txt"), new File("conf/modele_jeunes.xlsx"), true, false);
	}
}
