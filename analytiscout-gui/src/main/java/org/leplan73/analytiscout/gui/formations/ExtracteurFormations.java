package org.leplan73.analytiscout.gui.formations;
...
public class ExtracteurFormations extends Dialogue implements LoggedDialog, GuiCommand {

	private final JPanel contentPanel = new JPanel();

	private JTextField txfCodeStructure;
	private Logger logger_ = LoggerFactory.getLogger(ExtracteurFormations.class);
	private JFileChooser fcSortie;
	private File fSortie = new File(Preferences.lit(Consts.REPERTOIRE_SORTIE, "données", false),"formations.xls");
	 ...
	 * Create the dialog.
	 */
	public ExtracteurFormations() {
		super();
		setTitle("Extracteur des formations");
		...
		new Thread(() -> {
			...
			if (ret) {
				try {
					...
					Date debut = debutModel_.getValue();
					Date fin = finModel_.getValue();
					EngineExtracteurFormations en = new EngineExtracteurFormations(progress, logger_);
