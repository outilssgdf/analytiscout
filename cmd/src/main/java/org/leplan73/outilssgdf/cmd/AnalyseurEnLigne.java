...
public class AnalyseurEnLigne extends CommonParamsIntranet {
	...
	public void run(CommandLine commandLine) throws CmdLineException
	{
		...
		try {
			...
			en.go(identifiant,motdepasse, new MarkableFileInputStream(new FileInputStream(batch),0), new MarkableFileInputStream(new FileInputStream(modele),0), structures, age, "tout_responsables", recursif, psortie, anonymiser, garder);
		} catch (Exception e) {
