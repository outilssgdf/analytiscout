...
import org.leplan73.analytiscout.formatage.ArchiveCsvFormateur;
...
public class EngineGenerateurArchiveCsv extends EngineConnecte {

	public EngineGenerateurArchiveCsv(Progress progress, Logger logger) {
	...
	private boolean gopriv(ExtractionAdherents app, String identifiant, String motdepasse, File props, File sortie, int structure) throws ClientProtocolException, IOException, JDOMException, InvalidFormatException, ExtractionException
	{
		...
		ArchiveCsvFormateur f = new ArchiveCsvFormateur();
		
	    f.genereEmail(x.getUnites(), x.getAdherents(), x.getColonnes(), properties, zipOut);
	    zipOut.flush();
	    zipOut.close();
		return true;
	}
