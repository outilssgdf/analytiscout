		...
		new Thread(() -> {
			...
			if (ret) {
				try {
					...
					en.go(txfIdentifiant.getText(), new String(txfMotdepasse.getPassword()), new ResetableFileInputStream(new FileInputStream(fBatch)), new ResetableFileInputStream(new FileInputStream(fModele)), structures, chkAge.isSelected(), "tout_responsables", true, psortie ,false, chkGarderFichiers.isSelected());
					btnOuvrir.maj();
				} catch (Exception e) {
